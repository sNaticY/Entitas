...
- `Bootstrap` composes the runtime `ContextSchema` with `SharedContext.CreateSchemaBuilder().AddPlayerAssembly().AddHealthAssembly()`, registers it through generated `contexts.RegisterShared(schema)`, and drives the scene controller.
